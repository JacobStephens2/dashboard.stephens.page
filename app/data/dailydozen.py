...
async def accounts() -> list[Account]:
    rows = await fetch_sqlite(NAME, """
        SELECT u.email, u.created_at, MAX(ud.updated_at) AS last_activity
        FROM users u
    ...
    """)
    return [
        Account(app=LABEL, email=r['email'], created_at=r['created_at'],
                last_activity=r['last_activity'], activity_label=ACTIVITY)
        for r in rows
    ]
